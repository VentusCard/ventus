        ...
        <div className={`mx-auto relative ${isMobile ? 'max-w-full' : 'max-w-2xl'}`}>
          <Card className="border-0 shadow-xl bg-white/95 backdrop-blur-sm">
            <CardHeader className="text-center p-4 md:p-6">
              ...
              <p className="text-sm md:text-base text-slate-600 leading-relaxed">Fill out the form below to contact us at hello@ventuscard.com and we'll get back to you soon!</p>
            </CardHeader>
              ...
              <form className="space-y-4 md:space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input name="name" placeholder="Enter your full name" className="h-12 text-base" required />
                  </div>
                  <div>
                    <label className="text-slate-700 font-medium mb-2 block text-sm md:text-base">Email Address</label>
                    <Input name="email" type="email" placeholder="your.email@example.com" className="h-12 text-base" required />
                  </div>
                </div>

                <div>
                  <label className="text-slate-700 font-medium mb-2 block text-sm md:text-base">Subject</label>
                  <Input name="subject" placeholder="How can we help?" className="h-12 text-base" required />
                </div>

                <div>
                  <label className="text-slate-700 font-medium mb-2 block text-sm md:text-base">Message</label>
                  <Textarea name="message" placeholder="Please share your questions or feedback..." className="min-h-32 resize-none text-base" required />
                </div>

                <div className="flex justify-center pt-2">
                  <Button type="button" onClick={handleMailTo} variant="outline" className="h-12 border-2 border-blue-600 text-blue-600 hover:bg-blue-50 font-semibold rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-[1.02] px-6 md:px-8 text-sm md:text-base min-w-[200px]">
          ...
          {/* Success Overlay */}
          {showSuccessOverlay && <div className={`${isMobile ? 'fixed inset-0 z-50' : 'absolute inset-0 z-10'} bg-white/95 backdrop-blur-sm ${isMobile ? '' : 'rounded-xl border-0 shadow-xl'} flex items-center justify-center p-4`}>
              <div className="text-center p-6 md:p-8 max-w-md w-full">
                ...
                <Button onClick={handleCloseOverlay} className="bg-blue-600 hover:bg-blue-700 text-white h-12 px-6 text-base min-w-[120px]">
                  Close
                </Button>
              </div>
            </div>}
