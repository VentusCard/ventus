...
export function PipelineHeader({ opportunities, selectedGapId, onGapChange }: PipelineHeaderProps) {
  const navigate = useNavigate();
  const { getStatusCounts } = usePipelineStatus();
  
  const counts = getStatusCounts(selectedGapId || undefined);
  
  const totalMerchants = selectedGapId 
    ? opportunities.find(o => o.id === selectedGapId)?.merchantPartnerships.length || 0
    : opportunities.reduce((sum, o) => sum + o.merchantPartnerships.length, 0);

  const totalOpportunity = selectedGapId
    ? opportunities.find(o => o.id === selectedGapId)?.totalOpportunityAmount || 0
    : opportunities.reduce((sum, o) => sum + o.totalOpportunityAmount, 0);

  const handleExport = () => {
    const rows: string[] = ['Merchant,Gap,Category,Status,Est. Revenue,Target Users,Peak Quarter,Deadline'];
    
    const filteredOpps = selectedGapId 
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-2xl font-bold">Consumer Rewards Pipeline</h1>
          </div>
